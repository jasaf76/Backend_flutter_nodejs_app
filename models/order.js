...
Order.updateToDispatched = (id_order, id_delivery, result) => {
  ...
  db.query(
    sql,
    [id_delivery, "VERSCHICKT", new Date(), id_order],
    (err, res) => {
      if (err) {
...
(Order.findByStatus = (status, result) => {
  ...
  (Order.findDeliveryAndStatus = (id_delivery, status, result) => {
    const sql = `
    SELECT
        CONVERT(O.id, char) AS id,
        CONVERT(O.id_client, char) AS id_client,
   ...
   Order.updateToDelivered = (id_order, result) => {
    ...
    `;
     ...
     db.query(sql, ["GELIEFERT", new Date(), id_order], (err, res) => {
       if (err) {
         console.log("Error:", err);
         result(err, null);
       } else {
  ...
  Order.findClientAndStatus = (id_client, status, result) => {
    const sql = `
    SELECT
        CONVERT(O.id, char) AS id,
        CONVERT(O.id_client, char) AS id_client,
...
Order.updateToOnTheWay = (id_order, result) => {
  ...
  db.query(sql, ["AUF DEM WEG", new Date(), id_order], (err, res) => {
    if (err) {
      console.log("Error:", err);
      result(err, null);
    } else {
...
Order.create = (order, result) => {
  ...
  db.query(
    ...
    [
      order.id_client,
      order.id_address,
      "BEZAHLT", // 0: pendiente, 1: enviado, 2: en camino, 3: entregado
      Date.now(),
      new Date(),
      new Date(),
    ],
    (err, res) => {
      if (err) {
        ...
        console.log("Id del nuevo pedido:", res.insertId);
